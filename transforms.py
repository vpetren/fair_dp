...
def initialize_bert_transform(config):
    ...
    tokenizer = DistilBertTokenizerFast if 'distil' in config.model else BertTokenizerFast
    ...
    if 'distil' in config.model:
        return transform_distil
    else:
        return transform
