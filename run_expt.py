...
#from trustpilot_dataset import TrustPilotDataset
...
def main():
    ...
    parser.add_argument('--differentially_private', action='store_true')
    parser.add_argument('--noise_multiplier', type=float, default=0.4)
    parser.add_argument('--target_epsilon', type=int, default=10)
    parser.add_argument('--target_delta', type=int, default=1e-5)
    ...
    # Data
    if "trustpilot" in config.dataset:
    ...
    if config.differentially_private and not config.virtual_batch_size:
        config.virtual_batch_size = config.batch_size
    ...
    if config.differentially_private:
        if 'resnet' in config.model:
            algorithm.model = module_modification.convert_batchnorm_modules(algorithm.model)
            algorithm.model.to(config.device)
