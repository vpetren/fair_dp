...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train):
    ...
    # Differential privacy accounting parameter. Probability of not uploading privacy guarantees
    #DELTA = 1. / len(dataset['dataset'])
    DELTA = 1e-5
    ...
    if config.dataset in ['celebA', 'civilcomments', 'waterbirds']:
        epoch_y_pred = torch.argmax(epoch_y_pred, dim=1)
    ...
    if config.differentially_private:
        eps_log, _ = algorithm.optimizer.privacy_engine.get_privacy_spent(config.target_delta)
        results['eps'] = eps_log
...
def train(algorithm, datasets, general_logger, config, epoch_offset, best_val_metric):
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        general_logger.write(f'Validation {config.val_metric}: {curr_val_metric:.4f}\n')
...
def evaluate(algorithm, datasets, epoch, general_logger, config):
    ...
    for split, dataset in datasets.items():
        ...
        if config.dataset in ['celebA', 'civilcomments']:
            epoch_y_pred = torch.argmax(epoch_y_pred, dim=1)
